...
def adminindex(request):
    ...
    from datetime import date
    today = now().date()  # this is a `date` object

    user_count = User.objects.count()
    election_count = Election.objects.filter(electiondate__lt=today).count()
    distinct_candidate_count = Nominationpaper.objects.filter(
        status=1, cancelstatus=0
    ).values('loginid').distinct().count()

    ongoing_elections = Election.objects.filter(electiondate=today).count()
    upcoming_elections = Election.objects.filter(electiondate__gt=today).count()
    completed_elections = Election.objects.filter(electiondate__lt=now().date()).count()

    all_elections = Election.objects.all()
    election_list = []

    for election in all_elections:
        if election.electiondate > today:
            status = 'upcoming'
        elif election.electiondate < today:
            status = 'completed'
        else:
            status = 'ongoing'
...
import random
from .utils import decrypt_email
...
def loginindex(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if email_input == "electionofficer@gmail.com" and password_input == "Electionofficer@123":
            ...
            if check_password(password_input, found_user.password):
                ...
                request.session['usertype'] = found_user.usertype  
...
def userregistration(request):
    if request.method == 'POST':
        ...
        if form1.is_valid() and form2.is_valid():
            email_input = form2.cleaned_data['email']
            ...
            # Save login with encrypted email
            login_instance = form2.save(commit=False)
            login_instance.usertype = "USER"
            # login_instance.email =encrypt_email(login_instance.email)
            login_instance.save()
    ...
    if request.method == 'POST':
        ...
        if str(session_otp) == entered_otp:
            ...
            if usertype == 'USER':
                ...
                request.session['can_id'] = login_id  
...
def userhome(request):
    total_elections_count = Election.objects.count()
    last_seen_count = request.session.get('last_seen_election_count', 0)
    new_elections_count = max(total_elections_count - last_seen_count, 0)

    return render(request, 'userhome.html', {
        'new_elections_count': new_elections_count
    })
...
def candidatenotificationview(request):
     ...
     if not a:
        messages.error(request,'You are not a valid Candidate,please login and try again')
        return redirect('login')
     user=Notifications.objects.all()
...
def adminelectionview(request):
    ...
    return render(request, 'adminelectionview.html', {
        'user': user,
...
#     request.session['election_last_viewed_date'] = str(now().date())
#     user = Election.objects.all()
...
def userelectionview(request):
    ...
    # When user opens election view, mark as "seen" by updating session
    request.session['last_seen_election_count'] = Election.objects.count()
...
def electionview(request):
    ...
    # Prepare election list with days_left
    election_list = []
    for u in user:
        days_left = (u.electiondate - today).days
        election_list.append({
            'election': u,
            'days_left': days_left,
        })
...
def candidateinformationview(request):
     user_id = request.session.get('can_id')
     if not user_id:
        messages.error(request,'You are not a valid candidate,please login and try again')
        return redirect('login')
     user=Information.objects.all()
     return render(request,'candidateinformationview.html',{'user':user})
...
def publishresult(request, id):
    ...
    election = get_object_or_404(Election, id=id)
    election.publishstatus = 1
    election.save()

    # Get all nomination papers related to this election
    nominations = Nominationpaper.objects.filter(electionid=election)

    for nomination in nominations:
        # Get the User object from nomination
        user = nomination.loginid

        # Get the Login object using user's loginid foreign key
        login = user.loginid

        # Update the usertype
        login.usertype = 'USER'  # or whatever usertype value you want to set
        login.save()
...
def candidateviewallresult(request):
    ...
    for election in elections:
        ...
        # Decrypt all votes in this election and count them
        vote_counts = defaultdict(int)
        votes = VoteNow.objects.filter(electionid=election)
        for vote in votes:
            try:
...
from collections import defaultdict
from django.shortcuts import render, get_object_or_404
from .models import User, Election, Nominationpaper, VoteNow
from .utils import decrypt_candidate_id

def candidateviewresult(request, id):
    login_id = request.session.get('can_id')  
    if not login_id:
        messages.error(request, 'You must be logged in as a candidate to view this page.')
        return redirect('loginindex')

    userr = get_object_or_404(User, loginid__id=login_id)
    election = get_object_or_404(Election, id=id)

    # Get nominations for this candidate's ward
    nominations = Nominationpaper.objects.filter(
        electionid=election, cancelstatus=0, status=1, ward=userr.ward
    ).select_related('loginid', 'party')

    # Build candidate_id -> nomination map
    candidate_map = {nom.loginid.id: nom for nom in nominations}
    ...
    # Annotate nominations with vote counts
    for candidate_id, nomination in candidate_map.items():
        nomination.vote_count = vote_counts.get(candidate_id, 0)
...
def candidateelectionview(request):
    ...
    nominations = Nominationpaper.objects.filter(loginid=candidate)
    nominated_ids = nominations.values_list('electionid__id', flat=True)

    elections = Election.objects.all().order_by('-electiondate')
    ...
    return render(request, 'candidateelectionview.html', {
        'user': elections,
        'nominated_ids': list(nominated_ids),
        'current_date': current_date,
        'current_time': current_time,
    })
...
def deletecampaign(request,id):
    ...
    user.delete()
    return redirect('candidatecampaignview')
def othercampaigns(request):
    user_id = request.session.get('can_id')
    if not user_id:
        messages.error(request,'You are not a valid Candidate,please login and try again')
        return redirect('login')
    user=Campaign.objects.all()
    return render(request,'othercampaigns.html',{'user':user})
...
def nominationview(request):
    ...
    return render(request, 'nominationview.html', {'user': user, 'now': now})
def candidatenominationview(request):
     a=request.session.get('can_id')
     canid=get_object_or_404(User,loginid=a)
     user=Nominationpaper.objects.filter(loginid=canid)
     now = datetime.now()
     return render(request,'candidatenominationview.html',{'user':user,'now': now})

def cancelstatus(request,id):
    c=get_object_or_404(Nominationpaper,id=id)
    c.cancelstatus=1
    c.save()
    return redirect('nominationview')
def candidatecancelstatus(request,id):
    c=get_object_or_404(Nominationpaper,id=id)
    c.cancelstatus=1
    c.save()
...
def approvecandidate(request, id, userid):
    nomination = get_object_or_404(Nominationpaper, id=id)
    login_obj = get_object_or_404(Login, id=userid)

    login_obj.usertype = 'CANDIDATE'
    login_obj.save()

    nomination.status = 1
    nomination.save()

    # Get User and linked Login
    user_obj = nomination.loginid
    login_linked = user_obj.loginid

    # ✅ Directly use email if it is plain text
    candidate_email = login_obj.email
    candidate_email = decrypt_email(login_obj.email)
    # Send email
    subject = "Your Nomination has been Approved"
    message = (
        "Congratulations! Your nomination as a Candidate "
        "has been approved. You can now participate in the election."
    )
...
from django.core.mail import send_mail
# If your email is encrypted, import your decrypt_email function
...
def approveindependentcandidate(request, id, userid):
    ...
    if request.method == 'POST':
        ...
        if c:
            ...
            candidate_email = decrypt_email(b.email)

            # Send email
            subject = "Your Application has been Approved"
            message = (
                "Congratulations! Your application as an Independent Candidate "
                "has been approved. You can now participate in the election."
            )
            from_email = 'youremail@example.com'
            recipient_list = [candidate_email]
...
def adminapprovedcandidate(request):
    user = Nominationpaper.objects.filter(status=1, cancelstatus=0)
    ...
    return render(request, 'adminapprovedcandidate.html', {
        'user': user,
        'election_types': election_types,
        'wards': wards,
        'selected_election': selected_election,
...
from django.shortcuts import render, get_object_or_404, redirect
from .models import Chat, CandidateForum, Login, User
...
def userchat(request, id):
    ...
    # Get chats ordered by date
    chats = Chat.objects.filter(forumid=forum).order_by('currentdate')

    # Attach User object (name) to each chat message
    for chat in chats:
        try:
            user = User.objects.get(loginid=chat.senderid)
            chat.sender_user = user
        except User.DoesNotExist:
            chat.sender_user = None

    return render(request, 'userchat.html', {
        'form': form,
        'forum': forum,
        'chats': chats,
        'chat_user_id': logid.id,
...
def addreply(request, id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            user.reply = a
            user.save()
            messages.success(request, "Reply given successfully!")  # ✅ Add this line
...
def viewmyresult(request, id):
    ...
    user = get_object_or_404(User, loginid__id=login_id)
    nomination = get_object_or_404(Nominationpaper, id=id, loginid=user, status=1)

    election = nomination.electionid

    # Get all approved candidates in the same ward and election
    nominations_in_ward = Nominationpaper.objects.filter(
        electionid=election,
        ward=nomination.ward,
        status=1,
        cancelstatus=0
    ...
    # Prepare candidate map using loginid.id
    candidate_map = {nom.loginid.id: nom for nom in nominations_in_ward}
    vote_counts = defaultdict(int)
    ...
    # User's candidate id
    user_candidate_id = nomination.loginid.id

    my_votes = vote_counts.get(user_candidate_id, 0)

    # Find highest votes in the ward
    ward_vote_counts = [count for cid, count in vote_counts.items() if cid in candidate_map]
    max_votes = max(ward_vote_counts) if ward_vote_counts else 0

    # Determine result
    result_status = "Won" if my_votes == max_votes and my_votes != 0 else "Lost"
    ...
    return render(request, 'viewmyresult.html', context)

def candidateviewmyresult(request, id):
    login_id = request.session.get('can_id')
    if not login_id:
        messages.error(request, 'You are not a valid user, please login and try again.')
        return redirect('login')
    ...
    return render(request, 'candidateviewmyresult.html', context)
